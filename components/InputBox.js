...
function InputBox() {
    ...
    const [imageToPost, setImageToPost] = useState(null);//state to hold image
    ...
    const sendPost = (e) => {
        ...
        db.collection('posts').add({
            ...
            if (imageToPost) {
                const uploadTaskRef = ref(storage, `/posts/${doc.id}`);
                const uploadTask = uploadString(uploadTaskRef, imageToPost, 'data_url');

                removeImage();

                uploadTask.on(
                    'state_change',//maybe change state_changed
                    null,
                    ...
                    () => {
                        ref(storage, 'posts').child(doc.id).getDownloadURL().then(url => {
                            db.collection('posts').doc(doc.id).set({
                                postImage: url
                            }, { merge: true })//if not it will delete all from collection
                        })
                    })
            }
        })
